class Hws::Connectors::Hypto::Payout < Hws::Connectors::Hypto
  ...
  def send_to_bank_account(request:)
    ...
    Hws::Connectors::Dto::PayoutResponse
      .new(reference_number: resp_data['reference_number'], beneficiary: beneficiary, account_holder: resp_data['transfer_account_holder'],
           amount: resp_data['settled_amount'], payment_type: resp_data['payment_type'], status: resp_data['status'], message: resp['message'],
  ...
  def send_to_upi_id(request:)
    ...
    resp_data = resp['data']
    beneficiary = Hws::Connectors::Dto::Beneficiary
                    .new(name: resp_data['transfer_beneficiary_name'], upi_id: resp_data['transfer_upi_id'], note: resp_data['transfer_note'])
    Hws::Connectors::Dto::PayoutResponse
      .new(reference_number: resp_data['reference_number'], beneficiary: beneficiary, account_holder: resp_data['transfer_account_holder'],
           amount: resp_data['settled_amount'], payment_type: resp_data['payment_type'], status: resp_data['status'], message: resp['message'],
           txn_time: Time.strptime(resp_data['txn_time'], '%Y-%m-%d %H:%M:%S'), meta: resp_data.slice(*META_RESPONSES))
  ...
  def status(reference_number:)
    ...
    beneficiary = Hws::Connectors::Dto::Beneficiary
                    .new(name: resp_data['transfer_beneficiary_name'], account_number: resp_data['transfer_account_number'],
                         account_ifsc: resp_data['transfer_account_ifsc'], upi_id: resp_data['transfer_upi_id'], note: resp_data['transfer_note'])
    Hws::Connectors::Dto::PayoutResponse
      .new(reference_number: resp_data['reference_number'], beneficiary: beneficiary, account_holder: resp_data['transfer_account_holder'],
           amount: resp_data['settled_amount'], payment_type: resp_data['payment_type'], status: resp_data['status'], message: resp['message'],
           txn_time: Time.strptime(resp_data['txn_time'], '%Y-%m-%d %H:%M:%S'), meta: resp_data.slice(*META_RESPONSES))
